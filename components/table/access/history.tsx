...
class History {
  ...
  update(usecase: RowUsecase, action: ActionEnum, from: Access, to: Access) {
    const { success, abort } = sendAccessUpdateWithAbort(usecase, action, to); // Send update to the API
    this.#undoStack.push({ usecase, action, from, to, success, abort, timestamp: new Date() });
