...
import { sendAccessUpdateWithAbort } from '@/components/table/access/update';
import { incrementOpenIndex, resetOpenIndex, setOpenIndexById, useIsOpen } from '@/components/table/open';
...
export const SelectAccess = ({ row, usecase, action, id }: SelectAccessProps) => {
  ...
  useOnClickOutside(cellRef, resetOpenIndex);
  ...
  const updateAccess = useCallback(
    async (newAccess: Access) => {
      ...
      setIsLoading(true);
      setAccess(newAccess);
      accessHistory.update(usecase, action, previousAccess, newAccess);

      const { success } = sendAccessUpdateWithAbort(usecase, action, newAccess);
