...
const INVALID_ATTACHMENT_DOCUMENT_TYPES: DocumentTypeEnum[] = [DocumentTypeEnum.ROL_QUESTIONS];

const INVALID_MAIN_DOCUMENT_TYPES: DocumentTypeEnum[] = [DocumentTypeEnum.JOURNALFOERT, DocumentTypeEnum.ROL_ANSWERS];
...
export const isValidCase = ({ user, caseStatus, documentType, parent }: RowUsecase): boolean => {
  ...
  if (parent !== ParentEnum.NONE) {
    if (INVALID_ATTACHMENT_DOCUMENT_TYPES.includes(documentType)) {
      return false;
    }
  ...
  if (parent === ParentEnum.NONE && INVALID_MAIN_DOCUMENT_TYPES.includes(documentType)) {
    return false;
  }
